...
func main() {
	...
	defer close(ch) // Ensure channel is closed when main exits
	...
	logger.Init(
		...
		logger.AddChannelOutput(ch), // Add channel output for streaming logs
		logger.AddConsoleOutput(),   // Add console output for real-time viewing
	)
	...
	// Start a goroutine to read from the channel and print messages
	go func() {
		for msg := range ch {
			// Here you can handle the log messages as needed, e.g., print to console
			println("Log from channel:", msg)
		}
	}()
	...
	time.Sleep(5 * time.Second) // Wait to ensure all logs are processed before exiting
